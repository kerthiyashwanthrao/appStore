...
class AppStore extends Component {
  state = {
    ...
    updatedAppsList: appsList.filter(item => item.category === 'SOCIAL'),
  }
  ...
  updateOnSearch = () => {
    const {updatedAppsList, word} = this.state
    const updatedList = updatedAppsList.filter(item =>
      item.appName.toLowerCase().includes(word.toLowerCase()),
    )
    this.setState({updatedAppsList: updatedList})
  }

  clickTabItem = tabId => {
    this.setState({
      activeTabId: tabId,
      updatedAppsList: appsList.filter(item => item.category === tabId),
    })
  }

  render() {
    const {word, activeTabId, updatedAppsList} = this.state
    return (
      <div className="container">
        ...
        <ul className="appsContainer">
          {updatedAppsList.map(appDetails => (
